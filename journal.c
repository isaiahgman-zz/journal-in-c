	...
	//initial print before input is received to update
        for(int i = 0; i < count; i++){
                if(highlight == i + 1){
                        ...
                        attroff(A_REVERSE);
                }else{ mvprintw(i + 10, 10, "%s", list[i]); }
        }
        refresh();
		...
		//determines what to do based on which arrow key was pressed
			...
			case KEY_UP:
				...
				//this loops from bottom to top if option is the last and user presses down again 
                                if(highlight == count){ highlight = 1; }
				else{ ++highlight; }
                                break;
                        case 10:
...
void welcome(){
        erase();
        char *options[] = {"Create New Entry", "Read Existing Entries", "Exit"};
        int n_choices = sizeof(options) / sizeof(options[0]);
        int choice = printList(options, n_choices);
        if(choice == 1){ creatingEntry(); }
	else if(choice == 2){ readingEntry(); }
	else{ return; }
}
//Method for "Read Existing Entries" option
void readingEntry(){
	...
	FILE *fp;
	fp = fopen(userName, "r");
	//Count hold the number of lines in file
	int count = 0;
	//file is read into dataArray
	while(fgets(dataArray[count], 500, fp) != NULL){ ++count; }
	...
	for(int i = 0; i < j; i++){
		if(highlight == i + 1){
			...
			attroff(A_REVERSE);
		}else{ mvprintw((i+i)+10, 10, readList[i]); }
	}
        refresh();
	//while loop continues until enter is pressed
	while(1){
		...
		//determines what to do based on which arrow key was pressed
                switch(c){
                        case KEY_UP:
                                if(highlight == 1){ highlight = j; }
				else{ --highlight; }
                                break;
                        case KEY_DOWN:
                                if(highlight == j){ highlight = 1; }
				else{ ++highlight; }
                                break;
                        case 10:
                                choice = highlight;
...
//method that handles "Create New Entry" option
void creatingEntry(){
...
int main(){
	//inizialize standard window with ncurses
	initscr();	
	keypad(stdscr, TRUE);
